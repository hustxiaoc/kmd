...
function log(msg) {
    console.log("[%s] %s", color.cyan(TAG), msg);
}

function error(msg) {
    msg = util.format.apply(null,arguments);
    log(color.green("[ERROR]")+color.red.bold(msg));
}

function warn(msg) {
    msg = util.format.apply(null,arguments);
    log(color.green("[WARN]") + color.redBright(msg));
}

function info(msg) {
    msg = util.format.apply(null,arguments);
    log(color.greenBright(msg));
}
...
function wrapKISSY(code, options) {
        ...
        if(options.seajs) {
            minifyCode = '\rdefine(' + (options.moduleName ? util.format("'%s',%s",options.moduleName,requires) : '')  + 'function(require, exports, module) {' + minify(code) + '});';
            wrappedCode = '\rdefine(' + (options.moduleName ? util.format("'%s',%s",options.moduleName,requires ) : '') +  'function(require, exports, module) {\n' + code + '\n});';
        }else {
            minifyCode = '\rKISSY.add(' + (options.moduleName ? util.format("'%s',%s",options.moduleName,requires) : '')  + 'function(S ,require, exports, module) {' + minify(code) + '});';
            wrappedCode = '\rKISSY.add(' + (options.moduleName ? util.format("'%s',%s",options.moduleName,requires ) : '') +  'function(S ,require, exports, module) {\n' + code + '\n});';
        }
